...
export function StoreProvider({ children }) {
  ...
  const api = useMemo(() => {
    ...
    return {
      ...
      // استيراد قوائم التلاميذ دفعة واحدة: { "الثانية 1": { niveau: 2, eleves: ["..."] } }
      importerListes: (data) => setState((s) => {
        const classes = [...s.classes];
        const eleves = { ...s.eleves };
        Object.entries(data || {}).forEach(([nom, info]) => {
          const liste = (Array.isArray(info) ? info : info.eleves || []).map((n, i) => ({ id: uid("e"), nom: String(n) }));
          let cl = classes.find((c) => c.nom.trim() === nom.trim());
          if (!cl) {
            cl = {
              id: uid("c"), nom, niveau: (info && info.niveau) || 1,
              effectif: liste.length, salle: "", titulaire: null,
              regime: { lun: "plein", mar: "plein", mer: "plein", jeu: "plein", ven: "plein" },
            };
            classes.push(cl);
          } else {
            cl.effectif = liste.length;
          }
          eleves[cl.id] = liste;
        });
        return { ...s, classes, eleves };
      }),
